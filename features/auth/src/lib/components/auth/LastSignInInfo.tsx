...
type LastSignInContent = NonNullable<NonNullable<Dictionary['userNav']>['lastSignIn']>;
...
const DynamicIcon: React.FC<{ name: string, className?: string }> = ({ name, className }) => (
    ...
    <span className={`inline-block h-3 w-3 ${className || ''}`} title={`Icon: ${name}`}></span>
    // --- [FIN DE SANEAMIENTO DE CÓDIGO v1.2.0] ---
);

export function LastSignInInfo({ profile, content, locale }: LastSignInInfoProps) {
  const traceId = useMemo(() => logger.startTrace('LastSignInInfo_Lifecycle_v1.0'), []);
  useEffect(() => {
  ...
  if (!lastSignInAt) {
    logger.traceEvent(traceId, 'Guardián: No hay datos de último inicio de sesión. No se renderizará.');
    return null;
  }
  ...
  logger.traceEvent(traceId, 'Render: Mostrando datos de sesión.', { date: lastSignInDate, location, ip });
