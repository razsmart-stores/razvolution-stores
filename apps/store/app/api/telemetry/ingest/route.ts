...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!validation.success) {
      logger.warn('[API/Telemetry] Payload de ingesta inválido.', {
    ...
    const eventsToInsert: TablesInsert<'heimdall_events'>[] = events.map(
      (event) => ({
        // --- [FIN DE CORRECCIÓN SOBERANA v1.1.0] ---
    ...
    logger.success(
      `[API/Telemetry] ${events.length} eventos de telemetría ingeridos con éxito.`,
      { traceId }
    );
    ...
    logger.error('[API/Telemetry] Fallo catastrófico en la ingesta.', {
      error: errorMessage,
      traceId,
    });
