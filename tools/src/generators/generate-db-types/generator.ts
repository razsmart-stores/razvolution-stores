...
export async function generateDbTypesGenerator(tree: Tree) {
  logger.info(
    '🚀 Iniciando Generador de Tipos de Base de Datos razvolution...'
  );
  ...
  try {
    ...
    if (!generatedTypes || generatedTypes.length < 100) {
      throw new Error(
        'La salida de la CLI de Supabase parece vacía o incompleta.'
      );
    }
    ...
    logger.info(
      `✅ Tipos de base de datos generados con éxito en: ${outputPath}`
    );
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    logger.error(
      `🔥 Error catastrófico durante la generación de tipos de DB: ${errorMessage}`
    );

    if ('stderr' in (error as object)) {
      logger.error(
        `Salida de error de la CLI: ${(error as { stderr: string }).stderr}`
      );
    }
